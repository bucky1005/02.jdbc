...
/* 수업목표. Connection 객체를 통해 자바코드로 DB의 CRUD(DML 작업)를 수행할 수 있다. */
public class Application1 {
    public static void main(String[] args) {
        Connection con = getConnection();
        PreparedStatement pstmt = null;

        /* 설명. DML(insert, update, delete) 작업 시에는 반환 결과가 int값이 된다.(ResultSet을 쓰지 않는다.)
            그리고 기존의 조회일 때는 executeQuery()를 사용했지만 DML 작업 시에는 excuteUpdate()를 사용한다. */
        int result = 0;
        ...
        try {
            ...
            /* 설명. 입력할 값의 타입에 따라 setString, setInt 등으로 선택하고 (인덱스번호, 입력할 값)을 매개변수로 전달한다. */
            ...
            result = pstmt.executeUpdate();         // 자동 커밋 상태에서는 excuteUpdate되면 자동 커밋한다.(feat. 수동 커밋 변환)
